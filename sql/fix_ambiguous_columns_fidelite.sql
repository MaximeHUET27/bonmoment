-- Fix pour bug "column reference ambiguous"
-- À exécuter sur Supabase SQL Editor
-- Date : 2026-04-21
--
-- Cause : dans enregistrer_passage_fidelite, RETURNS TABLE déclare des colonnes
-- (seuil_passages, description_recompense, nb_passages_total, recompense_en_attente)
-- qui portent le même nom que des colonnes de tables interrogées dans le corps.
-- PostgreSQL lève "column reference is ambiguous" car il voit deux candidats :
-- la colonne de la table FROM et la variable de retour du RETURNS TABLE.
--
-- Corrections : 5 occurrences qualifiées avec alias de table dans 3 requêtes SELECT.
-- Toutes dans enregistrer_passage_fidelite — les 9 autres RPC sont indemnes.
...
BEGIN
  ...
  -- FIX #1 + #2 : alias pf pour lever l'ambiguïté avec les colonnes RETURNS TABLE
  -- seuil_passages et description_recompense
  ...
  IF p_mode_consultation THEN
    -- FIX #3 + #4 : alias cf pour lever l'ambiguïté avec nb_passages_total
    -- et recompense_en_attente qui sont aussi dans RETURNS TABLE
  ...
  ELSE
    -- FIX #5 : alias cf pour lever l'ambiguïté avec recompense_en_attente (RETURNS TABLE)
